...
plugins` 只有
[amap_http.py:401](../../plugins/china-trip-weaver/src/china_trip_weaver/providers/amap_http.py#L401)
写死 `"sortrule": "distance"` 一处；`fixture_count` 88（[test_providers.py:117](../../tests/test_providers.py#L117)）。
唯有 `git grep -n distance_meters -- plugins/china-trip-weaver/src/china_trip_weaver/providers` **不是**
0 命中：实测该目录内已有 12 处，分布在三个文件——
[amap.py:160](../../plugins/china-trip-weaver/src/china_trip_weaver/providers/amap.py#L160)（`_route` 方法
的路线距离 claim，与本书要改的 `_pois` 方法无关）、`mcp_stdio.py`（6 处）、`rail12306.py`（5 处，含
[rail12306.py:286](../../plugins/china-trip-weaver/src/china_trip_weaver/providers/rail12306.py#L286) 起的站
...
（[amap.py:47-113](../../plugins/china-trip-weaver/src/china_trip_weaver/providers/amap.py#L47)）逐行读过，
...
（[schema/trip.schema.json:735](../../plugins/china-trip-weaver/schema/trip.schema.json#L735)）且
`properties` 里没有 `distance_meters`（10 个 `required` 键逐一列出，无遗漏，
[trip.schema.json:736-747](../../plugins/china-trip-weaver/schema/trip.schema.json#L736)）。
`tests/test_providers.py::run_fixture`
（[test_providers.py:84-96](../../tests/test_providers.py#L84)）对每个 POI 类夹具的每个归一化项都用
`SchemaSubsetValidator.validate_fragment("#/$defs/poi", item)` 强校验，`additionalProperties=false`
分支
（[validate_trip.py:163-166](../../plugins/china-trip-weaver/src/china_trip_weaver/validate_trip.py#L163)）
...
在键存在于被测值里才递归
（[validate_trip.py:167-169](../../plugins/china-trip-weaver/src/china_trip_weaver/validate_trip.py#L167)），
...
（`_segment_request` 对整份 request 做 `copy.deepcopy`，[journey.py:768](../../plugins/china-trip-weaver/src/china_trip_weaver/journey.py#L768)，不区分该 atomic Trip 的日期范围是否真的覆盖这句话提到的车次）。
E003（「assumption 提到的车次号必须能在 Trip 里渲染出对应事实」）的检查是**逐 atomic Trip 独立跑**
的（`plan_trip()` 内部，[planning.py:479](../../plugins/china-trip-weaver/src/china_trip_weaver/planning.py#L479)），`plan_journey()` 对 atomic Trip 的处理是顺序 for 循环、任何一段抛异常就整体中止
（[journey.py:259-273](../../plugins/china-trip-weaver/src/china_trip_weaver/journey.py#L259)）。`[9/25 福州]`
...
（[test_locked_rail_services.py:226](../../tests/test_locked_rail_services.py#L226)）只单次调用
...
时 `rail_selection.select_service`（[rail_selection.py:29](../../plugins/china-trip-weaver/src/china_trip_weaver/rail_selection.py#L29)）的 `_matches_time` 会同时命中两行，`_locked_rail_candidate`
（[planning.py:1476](../../plugins/china-trip-weaver/src/china_trip_weaver/planning.py#L1476)）据此判定
...
管理者补记（2026-09-18）：`v0.7.0` 标签已于 2026-09-10 推送到远端，指向 2026-09-08 的发版提交 `d809ab1`；Release 页面于 2026-09-18 经领导同意事后补建，说明里写明是补建，「最新版本」标记仍在 v0.25.0。已关闭。
...
1. [tests/test_design_docs.py:20](../../tests/test_design_docs.py#L20) 的 `self.assertEqual(48, len(files))` 改成 `49`。
2. [docs/design/09-impl-map.md:66](../../docs/design/09-impl-map.md#L66) 前后，按字母序在 `build_scheduler_fixtures.py` 和 `qa_renderer_browser.py` 两行之间插入 `├── measure_coverage.py`。
...
管理者裁决（2026-09-18，验收时补记）：认可。执行记录写进了 PROGRESS「现状速览」一节末尾而不是文件末尾，发版迁移时由管理者一并迁入历史。暗卷：现役行程副本（第 11 版）经补坐标折回后跑餐饮，22 个时段有参考 17→18，唯一变化是 10/7 离岛后的晚餐锚到当晚住处（中山路一带的店），其余 21 个时段（含全部午餐）逐个不变；补坐标信封 `health.status` 为 `ready`（7 located、2 unresolved、0 provider_error）。

## 书 AS2「in-place-updates」（2026-09-18，第三十七波两本并行之一，worktree `.tmp/wt-as2` 分支 `in-place-updates`）：无

无。任务书「我替领导拍的板」一节把原子写的实现细节（同目录临时文件＋`os.replace`、失败时删临时文件、不加「输出不能等于输入」的限制、不做自动备份）与全部文案改法都定死，照做即可闭合任务 0 那条复现测试与任务 1 的五命令子进程测试。唯一需要现场判断的是任务书未点名的一处实现细节，判断为工程惯例而非设计分叉：`write_text_atomic` 失败路径里「先 `os.close` 描述符、再 `unlink` 临时文件」的顺序与写法，参照了仓库里已有的 `candidates.py::_stage_bytes` 同款「同目录临时文件＋`os.fchmod`＋失败时 `os.close` 再 `unlink`」写法，因为任务书只要求「失败时删临时文件、目标不动」这个结果，没有规定实现路径，按仓库既有惯例最省改动。全程没有遇到拿不准、需要管理者裁决的真实二义性。

管理者裁决（2026-09-18，验收时补记）：代码认可，文案三处由管理者改正：①「另存新文件」被写成「新文件此后也永不被覆盖」（README 两份与 plan SKILL），领导原意是另存时旧文件不动；②「`--output-json` 默认就是 `--journey`」不准确——该参数必填、没有默认值，改成「按约定就是 `--journey` 那个文件本身」（README 两份各三处、06 三处、mobility SKILL 四处、replan SKILL 一处，并写明两个输出参数都必填）；③顺手把 README 两份早已过时的「88 个合成服务商夹具」改成 93。暗卷：现役行程副本上按新约定原地串跑补坐标折回（11→12）与餐饮折回（12→13），拿旧版本号重跑报 `revision_conflict` 且字节不变，同名页面连渲两次字节相同、validate-html 0，目录里不留临时文件。注：领导当天 18:15 起已用 0.26.0 在新目录把现役行程更新到第 11 版（登记景点门票），旧约定下 Codex 多写了 `journey-r10-*`/`journey-r11-*` 两份文件，装上 0.27.0 后改为原地更新。
