...
| AMap Web Service（endpoint schema fingerprint） | POI、附近搜索（poi_around，餐饮参考用 `sortrule=weight` 取高德自身综合排序与 `business` 评分/人均/特色菜/菜系/营业时间）、geocode、walking/transit/driving/riding route matrix、逐日天气预报（weather，当天+3 天） | `AMAP_WEBSERVICE_KEY` | 不发 API；保留已有可信坐标或 static candidates；天气留 unknown | cached → keyless official map deep link/estimate → unknown | 401/403/429；v3/v4/v5 shape 与 GCJ-02；天气「成功但空」与多 forecasts 歧义都判 no_results |
