...
- **判定口径**：坐标解析走 `MobilityBackend.locate`（`mobility.py`，只解析坐标、不查路线矩阵，每个子 Trip 最多 12 个景点，住宿不限），与规划器共用同一套判定——`_poi_admin_matches` 的行政区匹配、`POI_NAME_SIMILARITY_MARGIN`（0.15）的名称相似度阈值、`POI_COORDINATE_CLUSTER_MAX_METERS`（300 米）的坐标聚集半径——一条不放宽。
