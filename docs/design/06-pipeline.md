...
- **锚点**：`dining.anchor_for` 在同一天先向前再向后找最近一个有坐标的 `poi`/`checkin`/`checkout`/`rest`/`lodging` 时段作圆心，遇到 `transport` 时段即停（换乘之后的一餐只在到达城市找圆心，换乘之前的只在出发城市找），`meal` 自己的占位 POI 永不当锚点；同一个坐标在一次 `plan_trip` 内只查一次 AMap。
