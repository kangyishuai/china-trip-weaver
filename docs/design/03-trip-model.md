...
`request.locked_rail_services`（可选，`#/$defs/lockedRailService` 数组）表达旅行者已购买并锁定、规划器不许另选的具体车次：`service_number` 与 `travel_date` 必填，`depart_time`（`HH:MM`）可选，仅用于在同一天同一车次号命中多行时消歧（典型如同城两站，例如「福州南站」与「福州站」）；`arrive_time`（`HH:MM`，同款正则）同样可选，用于两行同发不同到（例如「福州站」与「福州南站」）的场景，`depart_time` 与 `arrive_time` 可同时给出或各自单独给出。它是纯增量字段，不进 `request` 的 `"required"`，旧 request 一字不改继续有效。选车顺序、查不到时的占位腿与理由文案见 [第 06 篇 §3.3](06-pipeline.md#33-candidate-generation-与去重) 与 [ADR-0020](adr/0020-locked-service-assumption.md)。

`day.weather`（可选，`#/$defs/weatherForecast`，或 `null`）承载该日的天气预报：12 个字段全部必填——`forecast_date`（`YYYY-MM-DD`）、`adcode`（6 位数字字符串）、`city`、`day_text`、`night_text`、`temp_high_c`/`temp_low_c`（整数摄氏度）、`wind_day`/`wind_night`、`reported_at`（带 `+08:00` 偏移的 ISO 时间）、`advice`（可为空的字符串数组，逐条文案照原文渲染）与 `claim_id`（关联佐证这份预报的 claim，或为 `null`）。它是纯增量字段，不进 `day` 的 `"required"`，`additionalProperties` 仍为 `false`；旧 Trip 一字不改继续有效，某天缺这个键或其值为 `null` 都表示这天暂无预报。Trip 页与 Journey 页每天卡片如何显示这行天气、以及页面文案与该字段逐字一致由谁保证，见[第 07 篇 §2](07-renderer.md#2-页面信息架构)与[§7.1](07-renderer.md#71-结构一致性-errors)。规划器 `_plan_weather` 在 `active_mobility.mode == "live"` 时按当天写入这个字段（见[第 06 篇 §5.5](06-pipeline.md#55-天气标注)）；`weather_fold` 把独立发起的 `ctw weather` 查询结果折回既有 Trip/Journey 的同一字段（见[第 06 篇 §7.6](06-pipeline.md#76-天气折回)），本篇只固定形状。

`slot.dining`（可选，`#/$defs/diningReference`，或 `null`）承载该时段的附近餐饮参考：6 个字段全部必填——`queried_at`（带 `+08:00` 偏移的 ISO 时间）、`anchor_ref`（作为搜索圆心的实体或时段引用）、`anchor_name`、`radius_m`（整数米）、`search_url`（`#/$defs/url`，高德深链）与 `options`（0–3 项 `#/$defs/diningOption` 数组）。每个 `diningOption` 11 个字段全部必填——`provider_poi_id`、`name`、`cuisine`/`tag`/`opentime_today`/`address`（均可为 `null`）、`rating`（字符串，高德原始评分文案，不保证是纯数字）、`cost_cny`（可为 `null` 的人均价格）、`distance_m`（整数米）、`deep_links`（`#/$defs/url` 数组，至少一项，高德 marker 深链）与 `claim_id`。它是纯增量字段，不进 `slot` 的 `"required"`，`additionalProperties` 仍为 `false`；旧 Trip 一字不改继续有效，某时段缺这个键或其值为 `null` 都表示这个时段暂无餐饮参考查询。`request.dining_preferences`（可选，`{cuisine: string|null, avoid: string[]}`）承载用户的菜系偏好与忌口；两键都必填，本篇只固定形状，不约束谁读取或如何应用它。两页每个带 `dining` 键的时段如何显示这块参考、以及页面文案与该字段逐字一致由谁保证，见[第 07 篇 §2](07-renderer.md#2-页面信息架构)与[§7.1](07-renderer.md#71-结构一致性-errors)。数据从哪来（附近搜索、锚点选取、菜系过滤）由后续书接线。
