...
def build() -> List[Dict[str, Any]]:
    ...
    station_rows_same_city_ticket = copy.deepcopy(RAIL_TICKET)
    station_rows_same_city_ticket.update({
        "to_station": "上海南示例站", "to_station_telecode": "SNX", "arrive_time": "11:50",
    })
    station_rows_other_city_ticket = copy.deepcopy(RAIL_TICKET)
    ...
    fixtures.extend([
        ...
        fixture("amap", "engine_error", geocode_req, response({"status": "0", "info": "ENGINE_RESPONSE_DATA_ERROR", "infocode": "30001", "api": "geocode-v3"}), health="degraded", error_class="invalid_request", source=AMAP_MUTATION_SOURCE, captured_at=AMAP_CAPTURED_AT),
        fixture("amap", "invalid_params", amap_req, response({"status": "0", "info": "INVALID_PARAMS", "infocode": "20000", "api": "poi-v5"}), health="degraded", error_class="invalid_request", source=AMAP_MUTATION_SOURCE, captured_at=AMAP_CAPTURED_AT),
        fixture("amap", "access_too_frequent", geocode_req, response({"status": "0", "info": "ACCESS_TOO_FREQUENT", "infocode": "10004", "api": "geocode-v3"}), health="rate_limited", error_class="rate_limited", source=AMAP_MUTATION_SOURCE, captured_at=AMAP_CAPTURED_AT),
        fixture("amap", "server_busy", request("route", dict(route_base, travel_mode="walk")), response({"status": "0", "info": "SERVER_IS_BUSY", "infocode": "10016", "api": "route-walking-v3"}), health="degraded", error_class="upstream_5xx", source=AMAP_MUTATION_SOURCE, captured_at=AMAP_CAPTURED_AT),
    ...
    fixtures.extend([
        ...
        fixture("variflight", "price", request("flight", {"action": "price", "dep_city": "BJS", "arr_city": "SHA", "date": "2026-09-10", "subject_refs_by_service": {"XX1001": "leg-flight"}}), response(vari_live_body("getFlightPriceByCities", [vari_live_price()])), source=VARIFLIGHT_SOURCE, captured_at=VARIFLIGHT_CAPTURED_AT),
