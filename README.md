...
`ctw locate` finds POIs and lodgings that still have empty coordinates in a Trip or Journey — skipping the planner's meal placeholders (`poi-routine-meal-*`) — and resolves up to 12 per run through AMap, applying the same identity and coordinate-cluster checks the planner uses. Each entity gets one row with `status` `located`, `unresolved`, or `provider_error`; the `reason` is `credential_missing` when no AMap key is configured and `locate_no_result` when AMap was queried but nothing was located. It prints `LOCATE_COMPLETE` in `--output-json` mode and exits 0 when at least one entity got located, 2 when none did, 1 on a provider error. `ctw journey locate` folds that result back into an existing Journey the same two-step way as `ctw journey weather` — first `ctw locate --journey JOURNEY.json --output-json LOCATE.json`, then `ctw journey locate --journey JOURNEY.json --locate-result LOCATE.json --base-revision N --output-json JOURNEY.json` — filling in coordinates only for the entities that were still missing them (one that already has coordinates is left byte-for-byte unchanged), appending `claim_ids` the same deduplicating way `ctw mobility` does, and writing a `trigger=provider_change` patch per changed child Trip with the Journey's revision advancing by exactly one no matter how many Trips changed; it prints `JOURNEY_LOCATE_COMPLETE` and exits 0 on success, `JOURNEY_LOCATE_NOOP` and exits 2 without writing a file when nothing changes, and fails with `revision_conflict` (exit 1) on a stale `--base-revision`; the `--journey` input is never modified. Locating a lodging's coordinates first lets the arrival day's dinner in `ctw dining` anchor on it.
