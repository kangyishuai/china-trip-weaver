...
`ctw dining` writes one nearby-dining reference per lunch/dinner slot in a Trip or Journey: the anchor is the nearest same-day slot with known coordinates, searching backward then forward; an AMap `poi_around` search (`sortrule=weight`, AMap's own ranking) around that point at `--radius` (default 1500 m) keeps up to `--limit` (default 3) rated venues in AMap's order, skipping any without a `rating` or whose name/tag/keytag/rectag contains an `--avoid` word (repeatable); `--cuisine` seeds the search keywords in place of the default 「餐厅」. A day with no anchor slot gets `dining: null` and a `dining_no_anchor` unknown. It prints `DINING_COMPLETE` in `--output-json` mode and exits 0 when at least one slot got a reference, 2 when every slot came back empty. `ctw journey dining` folds that result back into an existing Journey the same two-step way as `ctw journey weather` — first `ctw dining --journey JOURNEY.json --output-json DINING.json`, then `ctw journey dining --journey JOURNEY.json --dining-result DINING.json --base-revision N --output-json JOURNEY.json` — writing a `trigger=dining` patch per changed child Trip and advancing the Journey's revision by exactly one no matter how many Trips changed; it prints `JOURNEY_DINING_COMPLETE` and exits 0 on success, `JOURNEY_DINING_NOOP` and exits 2 without writing a file when nothing changes, and fails with `revision_conflict` (exit 1) on a stale `--base-revision`; the `--journey` input is never modified.
