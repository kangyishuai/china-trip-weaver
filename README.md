...
Every file for one trip — the request, `candidates.json`, the resulting `trip.json`/`journey.json`, their rendered `.html`, and any `weather-<date>.json`, `dining-<date>.json`, or `*.progress.ndjson` — lives together in `plans/<name>/` under the project root that invoked this plugin, not scattered across that root. Pick `<name>` as a readable Chinese phrase or its pinyin, for example `plans/福建中秋国庆16天/`, and create the directory first if it does not exist yet. Replanning and every weather/dining/locate fold default to updating in place: `--output-json` is the same file as `--trip`/`--journey`, and its rendered page re-renders onto its own path too. Only a new trip, or an explicit request to keep a separate copy, writes a new revision file such as `plans/<name>/journey-r<N>.json`, which is itself never overwritten afterward. This convention does not apply to this repository's own [`demo/`](demo/) fixtures or test fixtures.
...
`ctw journey weather` folds a `ctw weather --output-json` result back into an existing Journey in two steps: first `ctw weather --journey JOURNEY.json --output-json WEATHER.json`, then `ctw journey weather --journey JOURNEY.json --weather-result WEATHER.json --base-revision N --output-json JOURNEY.json`. Each `forecasts[]` row now also carries a `query` key (the name actually queried); folding matches a row to a day by date and by `query` equal to the first segment of that day's `city` split with `split_city_names`. A `forecast` row writes `day.weather`; a `no_forecast` row nulls it and records a `weather_no_results` unknown; an `out_of_window` row, or a day with no matching row, is left untouched. If no day changes — including folding the same result twice — the command prints `JOURNEY_WEATHER_NOOP`, exits 2, and writes nothing. Otherwise every changed child Trip is folded in the same pass and the Journey's revision advances by exactly one, however many child Trips changed; the command prints `JOURNEY_WEATHER_COMPLETE`, exits 0, and writes the new revision to `--output-json`. A `--base-revision` that no longer matches the file's revision fails with `revision_conflict` and exits 1 without writing anything. The command only ever writes `--output-json`, which defaults to the same file as `--journey`, updating it in place.

`ctw dining` writes one nearby-dining reference per lunch/dinner slot in a Trip or Journey: the anchor is the nearest same-day slot with known coordinates, searching backward then forward but never across a `transport` slot (a meal after a transfer is anchored in the arrival city, one before it in the departure city); an AMap `poi_around` search (`sortrule=weight`, AMap's own ranking) around that point at `--radius` (default 1500 m) keeps up to `--limit` (default 3) rated venues in AMap's order, skipping any without a `rating` or whose name/tag/keytag/rectag contains an `--avoid` word (repeatable); `--cuisine` seeds the search keywords in place of the default 「餐厅」. A day with no anchor slot gets `dining: null` and a `dining_no_anchor` unknown; a dinner slot with no anchor and no later `transport` slot that day anchors on that night's lodging (`day.stay_id`) instead, since lunch is usually eaten away from wherever the day started. It prints `DINING_COMPLETE` in `--output-json` mode and exits 0 when at least one slot got a reference, 2 when every slot came back empty; without `--output-json` it prints one line per slot instead. `ctw journey dining` folds that result back into an existing Journey the same two-step way as `ctw journey weather` — first `ctw dining --journey JOURNEY.json --output-json DINING.json`, then `ctw journey dining --journey JOURNEY.json --dining-result DINING.json --base-revision N --output-json JOURNEY.json` — writing a `trigger=dining` patch per changed child Trip and advancing the Journey's revision by exactly one no matter how many Trips changed; it prints `JOURNEY_DINING_COMPLETE` and exits 0 on success, `JOURNEY_DINING_NOOP` and exits 2 without writing a file when nothing changes, and fails with `revision_conflict` (exit 1 without writing anything) on a stale `--base-revision`. The command only ever writes `--output-json`, which defaults to the same file as `--journey`, updating it in place.

`ctw locate` finds POIs and lodgings that still have empty coordinates in a Trip or Journey — skipping the planner's meal placeholders (`poi-routine-meal-*`) — and resolves them through AMap (at most 12 POIs per child Trip; lodgings are not capped), applying the same identity and coordinate-cluster checks the planner uses. Each entity gets one row with `status` `located`, `unresolved`, or `provider_error`: an `unresolved` row's `reason` is the same warning the planner would record for that entity, or `locate_no_result` when there is none; a `provider_error` row's `reason` is `credential_missing` when no AMap key is configured. It prints `LOCATE_COMPLETE` in `--output-json` mode and exits 0 when at least one entity got located, 2 when none did (a provider failure only marks rows `provider_error`), and 1 when an input file cannot be read or is invalid. `ctw journey locate` folds that result back into an existing Journey the same two-step way as `ctw journey weather` — first `ctw locate --journey JOURNEY.json --output-json LOCATE.json`, then `ctw journey locate --journey JOURNEY.json --locate-result LOCATE.json --base-revision N --output-json JOURNEY.json` — filling in coordinates only for the entities that were still missing them (one that already has coordinates is left byte-for-byte unchanged), appending `claim_ids` the same deduplicating way `ctw plan` does, recording an `unresolved` row as an unknown with its `reason` while leaving `provider_error` rows alone, and writing a `trigger=provider_change` patch per changed child Trip with the Journey's revision advancing by exactly one no matter how many Trips changed; it prints `JOURNEY_LOCATE_COMPLETE` and exits 0 on success, `JOURNEY_LOCATE_NOOP` and exits 2 without writing a file when nothing changes, and fails with `revision_conflict` (exit 1 without writing anything) on a stale `--base-revision`. The command only ever writes `--output-json`, which defaults to the same file as `--journey`, updating it in place. Locating a lodging's coordinates first lets the arrival day's dinner in `ctw dining` anchor on it.

`ctw replan`'s `refresh` event replaces one rail leg with a freshly queried service: run `ctw rail --output-json` first, then pass that file's path as `--rail-result`. `--rail-result` is required for a `refresh` event and rejected for every other event type. A refresh removes every existing claim whose `subject_ref` is the replaced leg before adding that service's current claims, so repeated refreshes do not accumulate stale evidence. When the event omits `service_number`, the default selection only considers same-day services that depart no earlier than the previous slot's end before picking the earliest arrival, raising `refresh_overlap` (naming the candidate count and that end time) only when none qualify; an explicit `service_number` that still matches more than one same-day row raises `refresh_service_ambiguous` unless the event's `arrive_at` or `depart_at` (either a full ISO timestamp or a bare `HH:MM`) picks one. A `suspend` event removes a leg that stopped running (a cancelled train, a suspended ferry crossing) together with its slot, budget-ledger line, and any now-orphaned claims in one patch, swapping the slot for a `free`- or `poi`-kind `replacement_slot`; the patch `trigger` is `disruption`. The replaced slot's `title` is rewritten to `起点 → 终点 铁路 车次号` unless the event supplies its own non-blank `title`; a blank one fails with `refresh_title`.
