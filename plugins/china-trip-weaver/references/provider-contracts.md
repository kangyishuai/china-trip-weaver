...
| 12306 MCP | `12306-mcp@0.3.10`, exact 8 tools | Station/direct/interline/route-stop rail data; seat `num="*"` means not yet on sale | 90s | Public query; dated 12306 deep link → unknown. |
...
| AMap | v5 POI, v3 geocode/walk/transit/drive, v4 ride fingerprints; v3 weather | POI (`poi`, optional `types`/`city_limit`), nearby search (`poi_around`; dining references sort by `sortrule=weight`, AMap's own ranking, and read each item's `business` rating/cost/tag/keytag/opening-hours), geocode, route matrix, daily forecast (`weather`: one `adcode` or `city` name, today + 3 days; an ambiguous name is `no_results`, never a guess) | 8s POI/geocode/weather; 12s route | No API call; deep link/static estimate → unknown; weather stays unknown. |
...
| AnySearch | MCP `tools/call name=search` JSON-RPC 2.0, Markdown `content[]` result | Optional destination-search supplement | 15s | Disabled without user key; no request is sent without one, and auto-registration is always rejected. |
