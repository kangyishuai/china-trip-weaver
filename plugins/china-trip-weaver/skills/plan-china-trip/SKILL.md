...
1. Require destinations, real dates/date range, and traveler count; require origin for cross-city travel. Record conservative defaults for soft preferences in `request.assumptions`.
2. Invoke `$research-china-destination` for dated candidates and claims. Save one JSON object with exactly `candidates_version`, `pois`, `lodgings`, `claims`, and `unknowns`; never put transport legs in this file.
...
4. Invoke `$schedule-china-trip` only after candidate and route-matrix data exist. A structured no-solution result is a valid outcome; never drop a hard constraint.
