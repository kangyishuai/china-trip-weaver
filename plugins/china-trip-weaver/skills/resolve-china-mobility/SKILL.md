...
- A `ctw weather --output-json` result can be folded back into an existing Journey with `ctw journey weather`: it matches each `forecasts[]` row to a day by date and `query`, writes `day.weather` (or a `weather_no_results` unknown), and advances the Journey's revision by exactly one no matter how many child Trips changed. The command only writes `--output-json`, which by convention is the `--journey` file itself, updated in place; a run that changes no day exits 2 and writes nothing, and a stale `--base-revision` fails closed with `revision_conflict` and also writes nothing.
- A meal slot's AMap-ranked nearby-dining reference is available the same way through `ctw dining`, anchored on the nearest same-day slot with known coordinates (searching backward then forward, never across a `transport` slot) within a 1500 m default radius, keeping up to 3 rated venues in AMap's own `sortrule=weight` order; when that search finds no anchor for a dinner slot and the day has no later `transport` slot, it anchors on that night's lodging (`day.stay_id`) instead, since lunches are usually eaten away from where the day started. `ctw journey dining` folds its result back into an existing Journey exactly like `ctw journey weather` — a `trigger=dining` patch per changed child Trip, revision advanced by exactly one, the command only writes `--output-json` (by convention the `--journey` file itself, updated in place), `JOURNEY_DINING_NOOP` exit 2 when nothing changes and writes nothing, `revision_conflict` on a stale `--base-revision` and writes nothing.
- A POI or lodging still missing coordinates can be resolved on its own with `ctw locate` (skipping the planner's `poi-routine-meal-*` placeholders, at most 12 POIs per child Trip with lodgings uncapped, the same identity and coordinate-cluster checks as candidate resolution above); `ctw journey locate` folds its result back into an existing Journey the same way — filling in only the entities that were still missing coordinates, a `trigger=provider_change` patch per changed child Trip, revision advanced by exactly one, the command only writes `--output-json` (by convention the `--journey` file itself, updated in place), `JOURNEY_LOCATE_NOOP` exit 2 when nothing changes and writes nothing, `revision_conflict` on a stale `--base-revision` and writes nothing. Locate a lodging's coordinates before the arrival day's `ctw dining` so its dinner slot has an anchor.
...
scripts/ctw journey weather --journey plans/<name>/journey.json --weather-result plans/<name>/weather.json --base-revision 1 --output-json plans/<name>/journey.json
scripts/ctw locate --journey plans/<name>/journey.json --output-json plans/<name>/locate.json
scripts/ctw journey locate --journey plans/<name>/journey.json --locate-result plans/<name>/locate.json --base-revision 2 --output-json plans/<name>/journey.json
scripts/ctw dining --journey plans/<name>/journey.json --output-json plans/<name>/dining.json
scripts/ctw journey dining --journey plans/<name>/journey.json --dining-result plans/<name>/dining.json --base-revision 3 --output-json plans/<name>/journey.json
```

Each fold above updates `plans/<name>/journey.json` in place; a chained sequence must read back the new `revision.number` after every step and pass it as the next fold's `--base-revision` (1, then 2, then 3 above). Only a new trip, or an explicit request to keep a separate copy, should write a new file name such as `journey-r<N>.json` instead.
