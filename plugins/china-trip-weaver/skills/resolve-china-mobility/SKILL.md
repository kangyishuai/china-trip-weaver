...
Each fold above defaults to updating `plans/<name>/journey.json` in place; a chained sequence must read back the new `revision.number` after every step and pass it as the next fold's `--base-revision` (1, then 2, then 3 above). Only a new trip, or an explicit request to keep a separate copy, should write a new file name such as `journey-r<N>.json` instead.
